<template>
  <div>
    <v-row class="mb-4">
      <v-col cols="12">
        <h1 class="text-h4 font-weight-bold text-primary">Invoices</h1>
      </v-col>
    </v-row>
    ...
    <!-- Empty State -->
    <v-card v-if="!loading && invoices.length === 0" class="pa-12 text-center empty-state" elevation="0">
      <v-icon icon="mdi-file-document-outline" size="80" color="surface-lighter"></v-icon>
      <h3 class="text-h5 mt-6 mb-3">No invoices found</h3>
      <p class="text-body-1 text-secondary mb-6">Create your first invoice to get started</p>
      <v-btn :to="{ name: 'InvoiceCreate' }" color="primary" size="large" elevation="2">
    ...
    <v-dialog v-model="deleteDialog.show" max-width="450">
      <v-card class="dialog-card">
        <v-card-title class="text-h6 pa-6 pb-4">Confirm Deletion</v-card-title>
        ...
        <v-card-actions class="pa-6 pt-4">
          ...
          <v-btn color="error" :loading="deleteDialog.loading" @click="deleteInvoice">
            Delete
          </v-btn>
        </v-card-actions>
      </v-card>
